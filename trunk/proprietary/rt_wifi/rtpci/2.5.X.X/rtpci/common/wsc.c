    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	{
#ifdef IWSC_SUPPORT
		if (IWSC_PeerEapolStart(pAd, pEntry) == FALSE)
		{
...
{		
	...
	else if (MsgType == WSC_MSG_EAP_FAIL)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("Receive EAP_Fail from WPS AP (WscState = %d)\n", pWscControl->WscState));
		...
		else if (pWscControl->WscState == WSC_STATE_RX_M2D)
		{
			...
			*/
			if (pAdapter->StaCfg.BssType == BSS_ADHOC)
			{
				...
				WscSendEapolStart(pAdapter, pWscControl->WscPeerMAC);
			}
#endif /* IWSC_SUPPORT */
		}
...
#ifdef SAMSUNG_EASY_CONFIG
			 || (pEntry && pEntry->bRaAutoWpsCapable)
#endif // SAMSUNG_EASY_CONFIG //
			 ))
#endif
		{
			pWscControl->WscActionMode = WSC_REGISTRAR;
...
#if 0
		/*
			Ignore this action for preventing to send unnecessary WSC NACK
		*/
			...
			MsgType == WSC_MSG_M8)
	{
		...
		if ((pAdapter->OpMode == OPMODE_AP) ||
			((pAdapter->OpMode == OPMODE_STA) && 
			 (pWscControl->bWscTrigger
#ifdef SAMSUNG_EASY_CONFIG
			  || pWscControl->EasyConfigInfo.bDoAutoWps
#endif // SAMSUNG_EASY_CONFIG //
			  )))
			bGoWPS = TRUE;
...
#ifdef IWSC_SUPPORT
				/*
					If peer registrar is limited UI device, we need to use PIN of registrar not own PIN.
				*/
				if (pAdapter->StaCfg.IWscInfo.bLimitedUIPeer == FALSE)
#endif // IWSC_SUPPORT //
#endif
				//if (pAdapter->StaCfg.BssType == BSS_INFRA) //zero question
				{
...
#endif // CONFIG_AP_SUPPORT //   
#ifdef CONFIG_STA_SUPPORT
						((pAdapter->OpMode == OPMODE_STA) && (pWscControl->bConfiguredAP == FALSE)) ||
#endif // CONFIG_STA_SUPPORT //
						(0))
					{
						OpCode |= WSC_OPCODE_MSG;
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_DONE)
			{
				...
				IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter)
				{
					...
					if ((pAdapter->StaCfg.BssType == BSS_ADHOC) &&
						...
						(pAdapter->StaCfg.IWscInfo.IWscSmpbcAcceptCount > 0))
					{
						pWscControl->WscStatus = STATUS_WSC_IBSS_WAIT_NEXT_SMPBC_ENROLLEE;
					}
					else
#endif // IWSC_SUPPORT //
					pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
...
Fail:
	...
	if (pWscControl->WscState == WSC_STATE_CONFIGURED)
	{
		...
		if (pEntry && pEntry->bIWscSmpbcAccept)
		{
			...
			WscDelListEntryByMAC(pAdapter, BSS0, pEntry->Addr);
		}
#endif // IWSC_SUPPORT //
		if (pWscControl->Wsc2MinsTimerRunning)
		{
...
#ifdef CONFIG_AP_SUPPORT
		if (pAd->OpMode == OPMODE_AP)
			WpsCalculateMIC(pAd, &Data[0], Len, pWpsCtrl, pAd->ApCfg.MBSSID[apidx].Bssid, MIC);
#endif // CONFIG_AP_SUPPORT //
#ifdef CONFIG_STA_SUPPORT
		if (pAd->OpMode == OPMODE_STA)
			WpsCalculateMIC(pAd, &Data[0], Len, pWpsCtrl, pAd->CurrentAddress, MIC);
#endif // CONFIG_STA_SUPPORT //
...
//	UCHAR 			Data[512];
	...
	if ( b_selRegistrar )
	{
		...
		// 5. Selected Registrar Config Methods
#ifdef IWSC_SUPPORT
		if (pAd->StaCfg.IWscInfo.bSelRegStart == FALSE)
		{
			if (pWpsCtrl->WscMode == WSC_PIN_MODE)
			{
				...
				if (pAd->StaCfg.IWscInfo.bLimitedUI)
				{
					selRegCfgMethods &= 0xFEFF;
				}
				else
				{
					selRegCfgMethods |= 0x0100;
				}
			}
			else
			{
...
#ifdef IWSC_SUPPORT
		if (pAd->StaCfg.IWscInfo.bSelRegStart)
		{
...
//	UCHAR			OutMsgBuf[512];		// buffer to create message contents
...
#ifdef SAMSUNG_EASY_CONFIG
	if (pAd->StaCfg.WscControl.EasyConfigInfo.bRaAutoWpsAp &&
		!pAd->StaCfg.WscControl.bWscTrigger)
	{
		...
		*/
		NdisMoveMemory(VendorExt, RALINK_OUI, 3);
		WpsCalculateMIC(pAd, &OutMsgBuf[0], Len, &pAd->StaCfg.WscControl, pAd->CurrentAddress, MIC);
		NdisMoveMemory(VendorExt + 3, MIC, LEN_KEY_DESC_MIC);
		/* 
			WPS Vendor Extension	
		*/
...
//	UCHAR 			Data[512];
		...
		!pAd->StaCfg.WscControl.bWscTrigger)
	{
		...
		*/
		NdisMoveMemory(VendorExt, RALINK_OUI, 3);
		WpsCalculateMIC(pAd, &Data[0], Len, &pAd->StaCfg.WscControl, pAd->CurrentAddress, MIC);
		NdisMoveMemory(VendorExt + 3, MIC, LEN_KEY_DESC_MIC);
		/* 
			WPS Vendor Extension	
		*/
	...
	IN  UCHAR       	BssIndex)
{
	...
	{
		pWscControl->WscEnrolleePinCode = GenerateWpsPinCode(pAd 
#ifdef CONFIG_AP_SUPPORT
															, bFromApCli
#endif /* CONFIG_AP_SUPPORT */
															, BssIndex);
...
#ifdef CONFIG_STA_SUPPORT
	pWscControl = &pAd->StaCfg.WscControl;
	bAdhocMode = (pAd->StaCfg.BssType == BSS_ADHOC);
#endif // CONFIG_STA_SUPPORT //
...
//			if (pAd->StaCfg.BssType == BSS_INFRA) //zero question
			{
		...
		else
		{
			if (pWscControl->WscMode == WSC_SMPBC_MODE)
				pWscControl->WscStatus = STATUS_WSC_SMPBC_TOO_MANY_REGISTRAR;
			else
			...
			DBGPRINT(RT_DEBUG_OFF, ("WscPBCExec --> AP list is %d, stop WPS process!\n", pWscControl->WscPBCBssCount));
		...
		if ((bFound == TRUE) && (bSelReg == TRUE))
			{							
				...
				if (bSameAP == FALSE)
				{
					...
					RTMPZeroMemory(&(ApUuidBssid[index].MacAddr[0]), MAC_ADDR_LEN);
					RTMPMoveMemory(&(ApUuidBssid[index].MacAddr[0]), &pInBss->MacAddr[0], MAC_ADDR_LEN);
	...
	OUT PWSC_PROFILE        pWscProfile)
{
	...
	if ((WSC_SCSTATE_UNCONFIGURED == pWscControl->WscConfStatus) &&
		((((OpMode & 0x0F) == AP_MODE) || (((OpMode & 0x0F) == STA_MODE) && ADHOC_ON(pAd))) && (OpMode & REGISTRAR_ACTION)))
	{
		...
		)
	{
		...
		while (Len > 0)
		{
			...
			if (pDPIDFromAP && be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID)
			{
				...
				DBGPRINT(RT_DEBUG_INFO, ("WscCheckWpsIeFromWpsPeer : DevicePasswordID = 0x%04x\n", DevicePasswordID));
#if 1
				*pDPIDFromAP = DevicePasswordID;
#else
					...
					else
					{
						...
						if (reason != 0) /* AP Reject STA */
						{
							if (pAd->StaCfg.BssType == BSS_INFRA)
							{
								...
								if (pAd->StaCfg.WscControl.EasyConfigInfo.ModuleType == MODULE_TV)
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	{		
		...
		*/
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			(pAd->StaCfg.WscControl.WscState == WSC_STATE_RX_M2D))
		{
...
#ifdef SAMSUNG_EASY_CONFIG
		BOOLEAN		bCancel;
		pAd->StaCfg.WscControl.EasyConfigInfo.bDoAutoWps = FALSE;
		RTMPCancelTimer(&pAd->StaCfg.WscControl.EasyConfigInfo.AdhocConnTimer, &bCancel);
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			(pAd->StaCfg.WscControl.EasyConfigInfo.ModuleType == MODULE_TV))
...
#ifdef CONFIG_STA_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		delEntryList(&pAd->StaCfg.WscControl.WscPeerList, pListEntry);
#endif // CONFIG_STA_SUPPORT //
	...
	IN	PUCHAR			pMacAddr)
{
	...
	pWscPeer = WscFindPeerEntry(pWscEnList, BSS0, pMacAddr);
	if (pWscPeer)
	{
		NdisGetSystemUpTime(&pWscPeer->receive_time);
	}
...
#if 0
VOID	WscFragmentEnable(
	IN  PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg)
{
	printk("bWscFragment = %d\n", pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.bWscFragment);
	return;
}

VOID	WscFragmentSize(
	IN  PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg)
{
	printk("WscFragSize = %d\n", pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscFragSize);
	return;
}
...
VOID	WscVersion2(
	...
	IN	PSTRING			arg)
{
	printk("Version2 = 0x%x\n", pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.RegData.SelfInfo.Version2);
	return;
}
...
VOID	WscTlvLen(
	...
	IN	PSTRING			arg)
{
	printk("TlvLen = %d\n", pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscV2Info.ExtraTlv.TlvLen);
	return;
}
...
{
	hex_dump("WscTlvData", 
			pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscV2Info.ExtraTlv.pTlvData, 
			pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscV2Info.ExtraTlv.TlvLen);
	return;
}
#endif // 0 //
