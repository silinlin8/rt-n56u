...
UCHAR ReptGetMuarIdxByCliIdx(RTMP_ADAPTER *pAd, UCHAR CliIdx)
{
	REPEATER_CLIENT_ENTRY *pReptEntry = NULL;
	...
	return (ReptOmacIdx - cap->RepeaterStartIdx) * 2;
}
	...
	PUCHAR pAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
		("%s:repeater_entry_time(%ld)\n", __func__, pReptCliEntry->CliTriggerTime));
				...
				if ((tr_entry->PortSecured != WPA_802_1X_PORT_SECURED) && (
					...
					if (!IS_REPT_LINK_UP(pReptCliEntry)) {
						...
						if (IS_AKM_SAE_SHA256(pApCliEntry->MlmeAux.AKMMap) || IS_AKM_OWE(pApCliEntry->MlmeAux.AKMMap)) {
							...
							if (
#ifdef DOT11_SAE_SUPPORT
								(IS_AKM_SAE_SHA256(pApCliEntry->MlmeAux.AKMMap) &&															sae_get_pmk_cache(&pAd->SaeCfg, pReptCliEntry->CurrentAddress, pApCliEntry->MlmeAux.Bssid, pmkid, pmk))
#endif
							) {
